...
source('Lawlor_Smith_Coarse.R')

trait.thres <- 1e-10
j.step <- 100 # for other simulations # had at 1000 for some others...
...
sym4.init.df <- data.frame(r1 = r, r2 = r, K1 = K, K2 = K,
                           ...
                           B11 = aii, B12 = aij, B21 = aij, B22 = aii,
                           w11 = w, w22 = w, 
                           m1 = m, m2 = m,
                           R1 = R, R2 = R, C1 = C, C2 = C)
...
set.steady <- safe.runsteady(set.temp[1,c("R1","R2","C1","C2")], func = ECD_model_LSC, parms = set.temp[1,c("r1","r2","K1","K2","e11","e12","e21","e22","B11","B12","B21","B22","w11","w22","m1","m2")])

sym4.eq.jac <- jacobian.full(y = set.steady$y, func = ECD_model_LSC, parms = set.temp[1,c("r1","r2","K1","K2","e11","e12","e21","e22","B11","B12","B21","B22","w11","w22","m1","m2")])
...
for(i in 1:dim(df)[1]){
  ...
  for(j in 2:mut.reps){
    ...
    if(j < j.step){
      delta.w11 <- 1 # arbitrary number greater than threshold value
      delta.w22 <- 1 # arbitrary number greater than threshold value
    }
    if(j > j.step){
      delta.w11 <- mean(diff(test[(j-j.step):(j-1),"w11"]))
      delta.w22 <- mean(diff(test[(j-j.step):(j-1),"w22"]))
    } 
    if(delta.w11 < trait.thres & delta.w22 < trait.thres) break 
    ...
    if(sub.sim.list.4[[j-1]]["feas.4sp"] == 1 & 
      ...
      parameters <- sub.sim.list.4[[j-1]][c("r1","r2","K1","K2","e11","e12","e21","e22","B11","B12","B21","B22","w11","w22","m1","m2")]
      ...
      if(sp == 1){
        new.ps["w11m"] <- new.ps["w11"] + mut
        #new.ps["w22m"] <- new.ps["w22"] - mut
        
        jac.mut <- jacobian.full(y = c(state, mC1 = 0), func = mutC1_ECD_model_LSC, parms = new.ps)
      ...
      if(sp == 2){
        ...
        jac.mut <- jacobian.full(y = c(state, mC2 = 0), func = mutC2_ECD_model_LSC, parms = new.ps)
      ...
      if(eigen.mut > 0 & all(new.ps >= 0) == TRUE){
        if(sp == 1){
          parameters["w11"] <- new.ps["w11m"]
          #parameters["w22"] <- new.ps["w22m"]
        }
        if(sp == 2){
          parameters["w22"] <- new.ps["w22m"]
          #parameters["w11"] <- new.ps["w11m"]
        ...
        out <- safe.runsteady(y = state, func = ECD_model_LSC, parms = parameters)
        
        # return steady states and maximum real eigenvalues
        if(any(out$y < abund.thres) == FALSE){
          
          eq.jac <- jacobian.full(y = out$y, func = ECD_model_LSC, parms = parameters) 
      ...
      if(eigen.mut < 0 & all(new.ps >= 0) == TRUE){
        ...
        sub.sim.list.4[[j]] <- c(sub.sim.list.4[[j-1]][1:24], 
...
sym4.eq.jac <- jacobian.full(y = set.steady$y, 
                             func = ECD_model_LSC, parms = set.temp[1,c("r1","r2","K1","K2","e11","e12","e21","e22","B11","B12","B21","B22","w11","w22","m1","m2")])
...
for(i in 1:dim(df)[1]){
  ...
  for(j in 2:mut.reps){
    ...
    if(delta.w11 < trait.thres & delta.B22 < trait.thres) break 
    ...
    if(sub.sim.list.4[[j-1]]["feas.4sp"] == 1 & 
      ...
      if(sp == 1){
        new.ps["w11m"] <- new.ps["w11"] + mut
        #new.ps["w12m"] <- new.ps["w12"] - mut
        
        jac.mut <- jacobian.full(y = c(state, mC1 = 0), func = mutC1_ECD_model_LSC, parms = new.ps)
        eigen.mut <- max(Re(eigen(jac.mut)$values)) 
      }
      #if(sp == 2){
      #  new.ps["B22m"] <- new.ps["B22"] + mut
      #  new.ps["B21m"] <- new.ps["B21"] - mut
      ...
      if(eigen.mut > 0 & all(new.ps >= 0) == TRUE){
        if(sp == 1){
          parameters["w11"] <- new.ps["w11m"]
          #parameters["w12"] <- new.ps["w12m"]
        }
        #if(sp == 2){
        #  parameters["B21"] <- new.ps["B21m"]
        #  parameters["B22"] <- new.ps["B22m"]
        #}
...
C1.init.df <- data.frame(r1 = r, r2 = r, K1 = K, K2 = K,
                         ...
                         B11 = aii, B12 = aij, m1 = m, w11 = w, 
                         R1 = R, R2 = R, C1 = C)

set.temp <- as.matrix(C1.init.df[1, ])

set.steady <- safe.runsteady(set.temp[1,c("R1","R2","C1")], func = ECD_model.C1.3sp_LSC, parms = set.temp[1,c("r1","r2","K1","K2","e11","e12","B11","B12","w11","m1")])

C1.eq.jac <- jacobian.full(y = set.steady$y, 
                           func = ECD_model.C1.3sp_LSC, parms = set.temp[1,c("r1","r2","K1","K2","e11","e12","B11","B12","w11","m1")])
...
for(i in 1:dim(df)[1]){
  ...
  for(j in 2:mut.reps){
    ...
    if(delta.w11 < trait.thres) break 
    ...
    if(sub.sim.list.3[[j-1]]["feas.3sp"] == 1 & 
      ...
      # extract parameters and state variables
      parameters <- sub.sim.list.3[[j-1]][c("r1","r2","K1","K2","e11","e12","B11","B12","w11","m1")]
      ...
      if(sp == 1){
        ...
        jac.mut <- jacobian.full(y = c(state, mC1 = 0), func = mut_ECD_model.C1.3sp_LSC, parms = new.ps) 
        eigen.mut <- max(Re(eigen(jac.mut)$values))
      }
      ...
      if(eigen.mut > 0 & all(new.ps >= 0) == TRUE){
        ...
        # run simulation with new parameters
        out <- safe.runsteady(y = state, func = ECD_model.C1.3sp_LSC, parms = parameters)
        
        # return steady states and maximum real eigenvalues
        if(any(out$y < abund.thres) == FALSE){
          
          eq.jac <- jacobian.full(y = out$y, func = ECD_model.C1.3sp_LSC, parms = parameters)
      ...
      if(eigen.mut < 0 & all(new.ps >= 0) == TRUE){
        ...
        sub.sim.list.3[[j]] <- c(sub.sim.list.3[[j-1]][1:16], 
                                 species = sp, mut.suc = 0, p.try = 1,
                                 sim.number = i, sequence = j)
      }
      if(all(new.ps >= 0) == FALSE){
...
evo.3 <- evol.symmetry.df.C1 %>% #filter(evol.symmetry.df.C1, mut.suc != 0) %>%
  select(sequence, B11, B12, w11, 
         R1, R2, C1,  
         max.Re.eigen = C1.max.eigen) %>%
  mutate(sim.type = "3_sp",
         special.C1R1 = B11*w11/(B11*w11+B12*(1-w11)),
         special.C1R2 = B12*(1-w11)/(B11*w11+B12*(1-w11)))

C1evo.4 <- C1evol.sim.df.4 %>% #filter(evol.sim.df.4, mut.suc != 0)
  select(sequence, B11, B12, B21, B22, w11, w22,
         R1, R2, C1, C2, 
         max.Re.eigen) %>%
  mutate(sim.type = "C1_4_sp",
         special.C1R1 = B11*w11/(B11*w11+B12*(1-w11)),
         special.C1R2 = B12*(1-w11)/(B11*w11+B12*(1-w11)),
         special.C2R1 = B21*(1-w22)/(B21*(1-w22)+B22*w22),
         special.C2R2 = B22*w22/(B21*(1-w22)+B22*w22))

evo.4 <- evol.sim.df.4 %>% #filter(evol.sim.df.4, mut.suc != 0)
  select(sequence, B11, B12, B21, B22, w11, w22,
         R1, R2, C1, C2, 
         max.Re.eigen) %>%
  # in LSCArthur model, w = 0 so I did not include it in the calculation of specialization
...
sim.dur3 <- dim(evo.3)[1]
sim.dur4 <- dim(evo.4)[1]
...
spec <- ggplot(bind_rows(evo.4[c(1,sim.dur4),],evo.3[c(1,sim.dur3),])) +
  ...
  geom_segment(aes(x = special.C2R1[1], xend = special.C2R1[2], y = special.C2R2[1], yend = special.C2R2[2]), 
               arrow = arrow(length=unit(0.4,"cm"), ends="last", type = "open"), color = cbbPalette[3], size = 1.1) +
  geom_segment(aes(x = special.C1R1[3], xend = special.C1R1[4], y = special.C1R2[3], yend = special.C1R2[4]), 
               arrow = arrow(length=unit(0.4,"cm"), ends="last", type = "open"), linetype = "dotted", color = cbbPalette[1], size = 1.1) + 
  geom_point(aes(x = special.C1R1[1], y = special.C1R2[1]), shape = 21, size = 5, fill = cbbPalette[6], color = cbbPalette[1]) +
  geom_point(aes(x = special.C2R1[1], y = special.C2R2[1]), shape = 21, size = 5, fill = cbbPalette[3], color = cbbPalette[1]) +
...
eff <- ggplot(evo.sym.df, aes(x = sequence, group = sim.type, linetype = sim.type)) +
  geom_line(aes(y = w11*B11 + B12*(1-w11)), color = cbbPalette[6], size = 1.2) + # C1
  geom_line(aes(y = w22*B22 + B21*(1-w22)), color = cbbPalette[3], size = 1.2) + # C2
...
p.square <- plot_grid(spec + theme(legend.position = "none"), 
                      ...
                      res.dens + theme(legend.position = "none"),
                      stab + theme(legend.position = "none"),

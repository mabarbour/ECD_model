...
set.steady <- safe.runsteady(set.temp[1,c("R1","R2","C1","C2")], func = ECD_model_Mac, parms = set.temp[1,c("r1","r2","K1","K2","e11","e12","e21","e22","a11","a12","a21","a22","m1","m2")])

sym4.eq.jac <- jacobian.full(y = set.steady$y, func = ECD_model_Mac, parms = set.temp[1,c("r1","r2","K1","K2","e11","e12","e21","e22","a11","a12","a21","a22","m1","m2")])
...
for(i in 1:dim(df)[1]){
  ...
  for(j in 2:mut.reps){
    ...
    if(sub.sim.list.4[[j-1]]["feas.4sp"] == 1 & 
      ...
      parameters <- sub.sim.list.4[[j-1]][c("r1","r2","K1","K2","e11","e12","e21","e22","a11","a12","a21","a22","m1","m2")]
      ...
      if(eigen.mut > 0 & all(new.ps >= 0) == TRUE){
        ...
        out <- safe.runsteady(y = state, func = ECD_model_Mac, parms = parameters)
        
        # return steady states and maximum real eigenvalues
        if(any(out$y < abund.thres) == FALSE){
          
          eq.jac <- jacobian.full(y = out$y, func = ECD_model_Mac, parms = parameters) 
      ...
      if(eigen.mut < 0 & all(new.ps >= 0) == TRUE){
        ...
        sub.sim.list.4[[j]] <- c(sub.sim.list.4[[j-1]][1:22], 
...
sym4.eq.jac <- jacobian.full(y = set.steady$y, 
                             func = ECD_model_Mac, parms = set.temp[1,c("r1","r2","K1","K2","e11","e12","e21","e22","a11","a12","a21","a22","m1","m2")])
...
set.steady <- safe.runsteady(set.temp[1,c("R1","R2","C1")], func = ECD_model.C1.3sp_Mac, parms = set.temp[1,c("r1","r2","K1","K2","e11","e12","a11","a12","m1")])

C1.eq.jac <- jacobian.full(y = set.steady$y, 
                           func = ECD_model.C1.3sp_Mac, parms = set.temp[1,c("r1","r2","K1","K2","e11","e12","a11","a12","m1")])
...
for(i in 1:dim(df)[1]){
  ...
  for(j in 2:mut.reps){
    ...
    if(sub.sim.list.3[[j-1]]["feas.3sp"] == 1 & 
      ...
      # extract parameters and state variables
      parameters <- sub.sim.list.3[[j-1]][c("r1","r2","K1","K2","e11","e12","a11","a12","m1")]
      ...
      if(eigen.mut > 0 & all(new.ps >= 0) == TRUE){
        ...
        # run simulation with new parameters
        out <- safe.runsteady(y = state, func = ECD_model.C1.3sp_Mac, parms = parameters)
        
        # return steady states and maximum real eigenvalues
        if(any(out$y < abund.thres) == FALSE){
          
          eq.jac <- jacobian.full(y = out$y, func = ECD_model.C1.3sp_Mac, parms = parameters)
      ...
      if(eigen.mut < 0 & all(new.ps >= 0) == TRUE){
        ...
        sub.sim.list.3[[j]] <- c(sub.sim.list.3[[j-1]][1:15], 
                                 species = sp, mut.suc = 0, p.try = 1,
                                 sim.number = i, sequence = j)
      }
      if(all(new.ps >= 0) == FALSE){
...
evo.3 <- evol.symmetry.df.C1 %>% #filter(evol.symmetry.df.C1, mut.suc != 0) %>%
  ...
  mutate(sim.type = "3_sp",
         special.C1R1 = a11/(a11+a12), #a11*w11/(a11*w11+a12*(1-w11)),
         special.C1R2 = a12/(a11+a12)) #a12*(1-w11)/(a11*w11+a12*(1-w11)))
...
C1evo.4 <- C1evol.sim.df.4 %>% #filter(evol.sim.df.4, mut.suc != 0)
  ...
  mutate(sim.type = "C1_4_sp",
         special.C1R1 = a11/(a11+a12), #a11*w11/(a11*w11+a12*(1-w11)),
         special.C1R2 = a12/(a11+a12), #a12*(1-w11)/(a11*w11+a12*(1-w11)),
         special.C2R1 = a21/(a21+a22), #a21*(1-w22)/(a21*(1-w22)+a22*w22),
         special.C2R2 = a22/(a21+a22)) #a22*w22/(a21*(1-w22)+a22*w22))

evo.4 <- evol.sim.df.4 %>% #filter(evol.sim.df.4, mut.suc != 0)
  select(sequence, a11, a12, a21, a22, #w11, w22,
         R1, R2, C1, C2, 
         max.Re.eigen) %>%
  # in MacArthur model, w = 0 so I did not include it in the calculation of specialization
...
spec <- ggplot(evo.sym.df %>% filter(sequence %in% c(1,sim.dur))) +
  ...
  geom_segment(aes(x = special.C1R1[3], xend = special.C1R1[4], y = special.C1R2[3], yend = special.C1R2[4]), 
               arrow = arrow(length=unit(0.4,"cm"), ends="last", type = "open"), linetype = "dotted", color = cbbPalette[6], size = 1.1) + 
...
eff <- ggplot(evo.sym.df, aes(x = sequence, group = sim.type, linetype = sim.type)) +
  geom_line(aes(y = a11 + a12), color = cbbPalette[6], size = 1.2) + # C1
  geom_line(aes(y = a22 + a21), color = cbbPalette[3], size = 1.2) + # C2

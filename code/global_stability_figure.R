...
Rx <- seq(0.1,5,0.1) # manipulating different Resource densities to solve R isocline.
Riso <- expression(r1*(K1-Rx)*(h11*Rx*aEFF)/K1*aEFF) # set R = 0 and solved algebraically
RisoStable <- eval(Riso)

### create consumer isocline
Ciso <- expression(m1 / (e11 - h11*m1)*aEFF) # set C = 0, and solved algebraically.
...
## copied from Speciation and Food webs "ecd attack rate manip.R"

## Copied Rmd file from Ch5_e_manip in Food webs file. Need to modify for Consumer-Resource models

## The code below illustrates how increasing the predator attack rate of the Rozenburg-MacArthur consumer-resource model alters the stability of the system.
...
rm1 <- ode(i.state,1:Time, ECD_model, p.rm1) # run the experiment 
...
par(mfrow=c(2,2)) # plot graphs side-by-side
...
plot(Rx, RisoStable, type = "l", ylab = "Consumer density", ylim=0:1, xlab="Resource density")
abline(v=CisoStable, lty = 2, col =2) 
...
#legend("right", c("Resource","Consumer"), lty=1:2, col=1:2, bty="n", cex = 0.8)
...
Time <- 300 # same pattern at larger time values, but I adjusted it to see the pattern more clearly
...
plot(Rx,RisoStable, type = "l", ylab = "Consumer density", ylim=0:1,
...
param.seq <- seq(1.3,2,length=50) # choose range of parameters
a <- 1.3

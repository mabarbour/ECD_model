...
```{r tradeoff, fig.pos="H", fig.cap="\\label{fig:tradeoff}**Evolutionary tradeoffs in consumer attack rates.** In each foraging scenario, I explored the effects of three different tradeoffs: (i) intermediate combinations of attack rates (a~ii~, a~ij~) are higher than the extremes (green line, n > 1); (ii) extreme combinations of attack rates are higher than intermediate investments (orange line, n < 1); and (iii) all combinations of attack rates have the same total attack rate (blue line, n = 1). Points corresponding to attack rates at the beginning of the simulation. Note that C~i~ was always pre-adapted to R~i~ (a~ii~>a~ij~)."}

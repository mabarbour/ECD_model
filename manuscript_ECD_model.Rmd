...
*Data accessibility*: No new data were used. All code used to reproduce this manuscript and simulations within are publically available on GitHub (https://github.com/mabarbour/ECD_model) and have been archived with Zenodo (https://zenodo.org/badge/latestdoi/79349192).
...
*Manuscript elements*: Figures 1-4; Online appendices S1-4 (including figures S1-3 in appendix S4). 
...
This first scenario is the starting point for many models of resource competition [@MacArthur1972]; however, it does not reflect many food webs where consumers are mobile and their foraging behavior links resources that occur in different habitats [@Holt1984]. 
...
where consumer $j$'s feeding rate on resource $i$ is influenced by the abundance of each resource; saturates as resource abundances increase (due to handling time $h_{ij}$); and consumer habitat preferences are modified by the relative abundance of resources, such that: $W_{ij}=w_{ij}R_i/(w_{1,j}R_1+w_{2,j}R_2)$.<!--$W_{ij}=\frac{w_{ij}R_i}{w_{1,j}R_1+w_{2,j}R_2}$.--> 
...
Specialization of consumer $j$ on resource $1$ is measured as $a_{1,j}/(a_{1,j}+a_{2,j})$<!--$\frac{a_{1,j}}{a_{1,j}+a_{2,j}}$-->, where a value of 0.5 is a complete generalist ($a_{1,j}=a_{2,j}$), and a value of 1 is a complete specialist ($a_{2,j}=0$).
...
All eigenvalues must be negative for the food web to be locally stable; however, the dominant eigenvalue, $\lambda_{\text{max}}$, is the one closest to positive infinity and ultimately determines food-web stability. 
If $\lambda_{\text{max}}>0$, then the food web is not locally stable. 
If $\lambda_{\text{max}}<0$, then the food web will return to equilibrium after a small perturbation (i.e., it is locally stable), with more negative values indicating a faster return time (i.e., greater stability). 
I chose to plot $-\lambda_{\text{max}}$ rather than $\lambda_{\text{max}}$, so that increasing values would indicate greater stability, which I felt was a more intuitive way to present the results.
<!-- old test: If $-\lambda_{max}>0$, then the food web will return to equilibrium after a small perturbation (i.e., it is locally stable), with more positive values indicating a faster return time.
...
I then repeated the simulation up to 10,000 times, which was sufficient for consumers to either reach an evolutionary stable strategy [ESS, @Smith1973] or an evolutionary limit (e.g., $a_{ij}/(a_{\text{1},j}+a_{\text{2},j})$<!--$\frac{a_{ij}}{a_{\text{1},j}+a_{\text{2},j}}$--> is constrained to a maximum of 1 and minimum of 0). 

<!--As with any model, the Adaptive Dynamics approach makes certain assumptions. First, it assumes a separation of time scales between ecological and evolutionary dynamics, an assumption that is becoming less tenable [@Hairston2005; @Hendry2016]. Second, it assumes there is no intraspecific variation within interacting species, despite the fact that intraspecific variation is a common feature of natural populations and often has important ecological consequences [@Bolnick2011; @Des_Roches2018].--> 

The Adaptive Dynamics approach I use here is one of several possible approaches for studying the eco-evolutionary dynamics of interacting species. A strength of this approach is that it enables me to gain analytical insight to the effects of character displacement in more realistic foraging scenarios.
...
In addition, I assume that there is no intraspecific variation within interacting species, despite the fact that intraspecific variation is a common feature of natural populations and often has important ecological consequences [@Bolnick2011; @Des_Roches2018].
Still, models that explicitly include resource dynamics inevitably show that resource competition results in character displacement, regardless of whether a quantitative genetic or Adaptive Dynamics approach is used [@Lawlor1976; @Taper1985].
Therefore, it should still serve as a good starting point for studying the ecological consequences of character displacement (but see \textbf{Caveats} section in the discussion).
...
All simulations were conducted in R [@R] and the code to reproduce these simulations is publically available on GitHub <!--(https://github.com/mabarbour/ECD_model) -->and has been archived with Zenodo <!-- (https://zenodo.org/badge/latestdoi/79349192)-->(please contact journal office for links).
...
tradeoff_plot <- bind_rows(concaveDN_df, linear_df, concaveUP_df) %>%
  ...
  scale_color_manual(name="Evolutionary\ntradeoff (n)", values=cbPalette) + theme_cowplot()
...
Note that the dip in stability toward a dominant eigenvalue of zero is an artifact of the two consumers becoming ecologically identical ($a_{11}=a_{21}=a_{12}=a_{22}$) on identical resources, and effectively become neutral species [@McPeek2019Neutral].
Although neutrality is an ecological possibility, it is never favored by evolution in these models since character displacement occurred across all the foraging scenarios I examined.<!-- never occurs this situation is not favored in any of the foraging scenarios I examined.
...
In fact, if the consumers were ecologically identical ($a_{1,j}=a_{2,j}$), then they are neutral species, as indicated by the dominant eigenvalue becoming zero [@McPeek2019Neutral]. 
This only occurs -->
...
As with the previous model, the dip in stability corresponds to the situation where consumers are effectively identical ($w_{11}a_{11}=w_{21}a_{21}=w_{12}a_{12}=w_{22}a_{22}$) and become neutral species, which is never favored in these models.
<!--Note that consumer neutrality only becomes possible with certain specific combinations of parameters in these models (e.g., $a_{1,j}=a_{2,j}$ or $w_{1,j}a_{1,j}=w_{2,j}a_{2,j}$).-->

It is worth noting that the scales are different between the left and right columns of Figure \ref{fig:plot_fig3}. When resources occur in different habitats (relative to the same habitat), the consumer's effective attack rate is lower because it is multiplied by the proportion $w_{ij}$. This results in relatively higher resource abundances and more stable food webs compared to when resources occur in the same habitat, and corresponds to the inherent stabilizing role of spatial heterogeneity in food webs [@Holt1984; @McCann2005].
...
LSs <- plot_grid(
  ...
  LS_resources + xlab("") + ylab("") +
    theme_cowplot() +
    theme(legend.position = "none", axis.text = element_text(size=10), axis.text.x = element_blank()),   
  Stability_LS + scale_x_continuous(labels = c(0,0.25,0.5,0.75,1)) + xlab("") + ylab("") + 
    theme_cowplot() +
...
```{r plot_fig3, echo=FALSE, fig.pos="H", fig.cap="\\label{fig:plot_fig3}**Effect of character displacement on food-web dynamics under different evolutionary tradeoffs and foraging scenarios.** Lines show predicted values when both consumers and resources are present. Different line colors correspond to different tradeoffs in attack rates (green, *n* = 1.15; blue, *n* = 1; orange, *n* = 0.85). Large circles (two consumers) and triangles (one consumer) correspond to the end points of the eco-evolutionary simulation for *C*~1~ (the choice to display *C*~1~ was arbitrary), whereas small shapes correspond to the starting points (only in stability panels). In both foraging scenarios, feeding rates increase linearly with resource abundance, but the equation for the effective attack rate is different."}
knitr::include_graphics("Fig_3_MacArthur_LawlorSmith.pdf")
```

```{r normalize-fig-3, include=FALSE}
## setup normalization
# from https://www.r-bloggers.com/custom-axis-transformations-in-ggplot2/
require(scales) # trans_new() is in the scales library
norm_Mac <- bind_rows(MacArthur_2C_2R_tradeoff_dynamics, MacArthur_1C_2R_tradeoff_dynamics) %>%
  mutate(eff_a = a11 + a12,
         RT = R1 + R2,
         Stability = -1*max.Re.eigen)
# easiest way to manually do this in the aesthetics

norm_LS <- bind_rows(LawlorSmith_2C_2R_tradeoff_dynamics, LawlorSmith_1C_2R_tradeoff_dynamics) %>%
  mutate(eff_a = w11*a11 + w12*a12,
         RT = R1 + R2,
         Stability = -1*max.Re.eigen)

## normalize plots MacArthur
Mac_attackNorm <- ggplot(tradeoff_df, aes(x=aii/(aii+aij), y=(aii+aij - min(norm_Mac$eff_a))/(max(norm_Mac$eff_a) - min(norm_Mac$eff_a)), color = factor(n))) + 
  geom_line() +
  geom_point(data = filter(MacArthur_displacements, Time=="End"), aes(x = a11/(a11+a12), y=(a11+a12 - min(norm_Mac$eff_a))/(max(norm_Mac$eff_a) - min(norm_Mac$eff_a)), fill=factor(n), shape=Competitor), size=3, color="white") + 
  ylab(expression(a[ii]+a[ij])) +
  scale_shape_manual(values=c(24,21)) +
  scale_color_manual(name="Tradeoff (n)", values=cbPalette)  +
  scale_fill_manual(name="Tradeoff (n)", values=cbPalette) +
...
Mac_resourcesNorm <- cbind.data.frame(select(tradeoff_df, n, aii, aij), MacArthur_2C_2R_tradeoff_dynamics) %>%
  ggplot(., aes(x=aii/(aii+aij), y=(R1+R2 - min(norm_Mac$RT))/(max(norm_Mac$RT) - min(norm_Mac$RT)), color = factor(n))) + 
  geom_line() +
  geom_line(data = cbind.data.frame(select(tradeoff_df, n, aii, aij), MacArthur_1C_2R_tradeoff_dynamics), alpha = 0.1) +
  geom_point(data = filter(MacArthur_displacements, Time=="End"), aes(x = a11/(a11+a12), y=(R1+R2 - min(norm_Mac$RT))/(max(norm_Mac$RT) - min(norm_Mac$RT)), fill=factor(n), shape=Competitor), size=3, color="white") +
  ylab(expression(R[i]+R[j])) +
  scale_shape_manual(values=c(24,21)) +
  scale_color_manual(name="Tradeoff (n)", values=cbPalette)  +
  scale_fill_manual(name="Tradeoff (n)", values=cbPalette) +
...
Stability_MacNorm <- cbind.data.frame(select(tradeoff_df, n, aii, aij), MacArthur_2C_2R_tradeoff_dynamics) %>%
  ggplot(., aes(x=aii/(aii+aij), y=(-1*max.Re.eigen - min(norm_Mac$Stability))/(max(norm_Mac$Stability) - min(norm_Mac$Stability)), color = factor(n))) + 
  geom_line() +
  geom_line(data = cbind.data.frame(select(tradeoff_df, n, aii, aij), MacArthur_1C_2R_tradeoff_dynamics), alpha = 0.1) +
  geom_point(data = filter(MacArthur_displacements, Time=="End"), aes(x = a11/(a11+a12), y=(-1*max.Re.eigen - min(norm_Mac$Stability))/(max(norm_Mac$Stability) - min(norm_Mac$Stability)), fill=factor(n), shape=Competitor), size=3, color="white", show.legend=FALSE) + 
  geom_point(data = filter(MacArthur_displacements, Time=="Begin"), aes(x = a11/(a11+a12), y=(-1*max.Re.eigen - min(norm_Mac$Stability))/(max(norm_Mac$Stability) - min(norm_Mac$Stability)), fill=factor(n), shape=Competitor), size=1.5, color="white") +
  ylab(expression("Stability "(-lambda[max]))) + 
  scale_shape_manual(values=c(24,21))  +
  scale_color_manual(name="Tradeoff (n)", values=cbPalette)  +
  scale_fill_manual(name="Tradeoff (n)", values=cbPalette) +
...
MacsNorm <- plot_grid(
  Mac_attackNorm + xlab("") + ylab(bquote(atop("Effective attack rate","("*a[1*","*j]+a[2*","*j]*")"))) +
    expand_limits(y = c(0,1)) +
    theme_cowplot() +
    theme(legend.position = "none", axis.text = element_text(size=10), axis.text.x = element_blank(), plot.title = element_text(size = 10), axis.title.y=element_text(vjust=0, size=12)), 
  Mac_resourcesNorm + xlab("") + ylab(bquote(atop("Resource abundance","("*R[1]+R[2]*")"))) + 
    expand_limits(y = c(0,1)) +
    theme_cowplot() +
    theme(legend.position = "none", axis.text = element_text(size=10), axis.text.x = element_blank(), axis.title.y=element_text(vjust=0, size=12)), 
  Stability_MacNorm + scale_x_continuous(labels = c(0,0.25,0.5,0.75,1)) + xlab("") + ylab(bquote(atop("Food-web stability","("*-lambda[max]*")"))) + 
    expand_limits(y = c(0,1)) +
    theme_cowplot() +
    theme(legend.position = "none", axis.text = element_text(size=10), axis.title.y=element_text(vjust=0, size=12)), 
  ncol=1, align='v', labels = c("","b)","c)"), hjust = -4, vjust = 0.5, label_size = 10
...
#Macs_title <- ggdraw() + draw_label("Resources: Same Habitat", fontface = 'bold', size = 10, hjust = 0.2)
plot_MacsNorm <- plot_grid(Macs_title, MacsNorm, ncol = 1, rel_heights = c(0.1, 1))

## LawlorSmith
LS_attackNorm <- ggplot(tradeoff_df, aes(x=aii/(aii+aij), y=(wii*aii+wij*aij - min(norm_LS$eff_a))/(max(norm_LS$eff_a) - min(norm_LS$eff_a)), color = factor(n))) + 
  geom_line() +
  geom_point(data = filter(LawlorSmith_displacements, Time=="End"), aes(x = a11/(a11+a12), y=(w11*a11+w12*a12 - min(norm_LS$eff_a))/(max(norm_LS$eff_a) - min(norm_LS$eff_a)), fill=factor(n), shape=Competitor), size=3, color="white") + 
  #geom_point(data = filter(LawlorSmith_displacements, Time=="Begin"), aes(x = a11/(a11+a12), y=w11*a11+w12*a12, fill=factor(n), shape=Competitor), size=1.5, color="white") +
  ylab(expression(w*a[ii]+(1-w)*a[ij])) +
  #xlab(expression(frac(a[ii],a[ii]+a[ij]))) +
  scale_shape_manual(values=c(24,21)) +
...
LS_resourcesNorm <- cbind.data.frame(select(tradeoff_df, n, aii, aij), LawlorSmith_2C_2R_tradeoff_dynamics) %>%
  ggplot(., aes(x=aii/(aii+aij), y=(R1+R2 - min(norm_LS$RT))/(max(norm_LS$RT) - min(norm_LS$RT)), color=factor(n))) + 
  geom_line() +
  geom_line(data = cbind.data.frame(select(tradeoff_df, n, aii, aij), LawlorSmith_1C_2R_tradeoff_dynamics), alpha = 0.1) +
  geom_point(data = filter(LawlorSmith_displacements, Time=="End"), aes(x = a11/(a11+a12), y=(R1+R2 - min(norm_LS$RT))/(max(norm_LS$RT) - min(norm_LS$RT)), fill=factor(n), shape=Competitor), size=3, color="white") + 
  #geom_point(data = filter(LawlorSmith_displacements, Time=="Begin"), aes(x = a11/(a11+a12), y=R1+R2, fill=factor(n), shape=Competitor), size=1.5, color="white") +
  ylab(expression(R[i]+R[j])) +
  #xlab(expression(frac(a[ii],a[ii]+a[ij]))) +
  scale_shape_manual(values=c(24,21)) +
...
Stability_LSNorm <- cbind.data.frame(select(tradeoff_df, n, aii, aij), LawlorSmith_2C_2R_tradeoff_dynamics) %>%
  ggplot(., aes(x=aii/(aii+aij), y=(-1*max.Re.eigen - min(norm_LS$Stability))/(max(norm_LS$Stability) - min(norm_LS$Stability)), color = factor(n))) + 
  geom_line() +
  geom_line(data = cbind.data.frame(select(tradeoff_df, n, aii, aij), LawlorSmith_1C_2R_tradeoff_dynamics), alpha = 0.1) +
  geom_point(data = filter(LawlorSmith_displacements, Time=="End"), aes(x = a11/(a11+a12), y=(-1*max.Re.eigen - min(norm_LS$Stability))/(max(norm_LS$Stability) - min(norm_LS$Stability)), fill=factor(n), shape=Competitor), size=3, color="white", show.legend=FALSE) +
  geom_point(data = filter(LawlorSmith_displacements, Time=="Begin"), aes(x = a11/(a11+a12), y=(-1*max.Re.eigen - min(norm_LS$Stability))/(max(norm_LS$Stability) - min(norm_LS$Stability)), fill=factor(n), shape=Competitor), size=1.5, color="white") +
  ylab(expression("Stability "(-lambda[max]))) + 
  scale_shape_manual(values=c(24,21))  +
  scale_color_manual(name="Tradeoff (n)", values=cbPalette)  +
  scale_fill_manual(name="Tradeoff (n)", values=cbPalette) +
...
LSsNorm <- plot_grid(
  LS_attackNorm  + xlab("") + ylab(bquote(w[1*","*j]*a[1*","*j]+w[2*","*j]*a[2*","*j])) + 
    expand_limits(y = c(0,1)) +
    theme_cowplot() +
    theme(legend.position = "none", axis.text = element_text(size=10), axis.text.x = element_blank(), plot.title = element_text(size = 10), axis.title.y=element_text(size=12)), 
  LS_resourcesNorm + xlab("") + ylab("") +
    expand_limits(y = c(0,1)) +
    theme_cowplot() +
    theme(legend.position = "none", axis.text = element_text(size=10), axis.text.x = element_blank()),   
  Stability_LSNorm + scale_x_continuous(labels = c(0,0.25,0.5,0.75,1)) + xlab("") + ylab("") + 
    expand_limits(y = c(0,1)) +
    theme_cowplot() +
    theme(legend.position = "none", axis.text = element_text(size=10)), 
  ncol=1, align='v', labels = c("","e)","f)"), hjust = -1.5, vjust = 0.5, label_size = 10
...
#LSs_title <- ggdraw() + draw_label("Resources: Different Habitat", fontface = 'bold', size = 10, hjust = 0.3)
plot_LSsNorm <- plot_grid(LSs_title, LSsNorm, ncol = 1, rel_heights = c(0.1, 1))

plot_fig3Norm <- plot_grid(plot_MacsNorm, plot_LSsNorm, ncol=2, rel_widths = c(1,0.9)) 
plot_fig3_labelNorm <- plot_fig3Norm + 
  draw_label("a)", x = 0.098, y = 0.925, fontface = 'bold', size = 10) + 
  draw_label("d)", x = 0.566, y = 0.925, fontface = 'bold', size = 10)
plot_fig3_subNorm <- add_sub(plot_fig3_labelNorm, expression(paste("Specialization ", bgroup("(",frac(a[ij],a[1*","*j]+a[2*","* j]),")"))), vjust = 0, size=12) 

save_plot(filename="Fig_3_MacArthur_LawlorSmith_Normalized.pdf", plot=plot_fig3_subNorm, base_height = 6.5, base_width = 6)
```
...
Specifically, I found that the two-consumer food web will become unstable<!--transition from having a locally stable equilibrium to a limit cycle--> under the following conditions (derived using Routh-Hurwitz criteria in Appendix S1):
...
```{r check-negative-eigenvalues, include=FALSE}
# MacArthur
filter(MacArthur_2C_2R_tradeoff_dynamics, a11/(a11+a12) == 0.5) %>%
  select(a11,a12,max.Re.eigen) %>%
  knitr::kable()

filter(MacArthur_2C_2R_tradeoff_dynamics, max.Re.eigen == 0) %>%
  select(a11,a12,max.Re.eigen) %>%
  knitr::kable()

with(MacArthur_2C_2R_tradeoff_dynamics, table(max.Im.eigen > 0)) %>% knitr::kable()

with(MacArthur_1C_2R_tradeoff_dynamics, table(max.Im.eigen > 0)) %>% knitr::kable()

# LawlorSmith
filter(LawlorSmith_2C_2R_tradeoff_dynamics, w11*a11/(w11*a11+w12*a12) == 0.5) %>%
  select(a11,a12,w11,w12,max.Re.eigen) %>%
  knitr::kable()

filter(LawlorSmith_2C_2R_tradeoff_dynamics, max.Re.eigen == 0) %>%
  select(a11,a12,w11,w12,max.Re.eigen) %>%
  knitr::kable()

with(LawlorSmith_2C_2R_tradeoff_dynamics, table(max.Im.eigen > 0)) %>% knitr::kable()

with(LawlorSmith_1C_2R_tradeoff_dynamics, table(max.Im.eigen > 0)) %>% knitr::kable()

with(McCann_2C_2R_tradeoff_dynamics, table(max.Im.eigen > 0)) %>% knitr::kable()

with(McCann_1C_2R_tradeoff_dynamics, table(max.Im.eigen > 0)) %>% knitr::kable()
```
...
<!-- stickleback paper about no differences in habitat: @Ormond2011. Interestingly, the species pair appears to be associated with lakes with low fish diversity...not clear with the trophic linkages are there -->
...
One of the criteria used to demonstrate ecological character displacement is that ``sites of sympatry [two consumers] and allopatry [one consumer] should not differ greatly in food $\textbf{[}$resource abundances$\textbf{]}$, climate, or other environmental features affecting the phenotype'' [@Schluter1992]. <!-- added full criteria -->
...
While it is theoretically possible that character displacement does not alter (or even increase) resource abundances, this was limited to the simplest, and arguably least realistic, foraging scenario and under tradeoffs that did not favor large displacements, and is thus less likely to detect in nature.
...
<!--Testing the prediction that character displacement decreases resource abundances could be done either experimentally, by putting consumers that have or have not undergone character displacement in a common resource setting, or with field data that statistically controls for other factors affecting resources.

Testing this prediction 
The most straightforward prediction to test is that character displacement decreases resource abundances.--> 

My predictions could be tested with an experiment that puts consumers that have and have not undergone character displacement in a common resource setting and comparing their effect on resource abundances.
One experiment with threespine stickleback has actually done this, although they focused more on the community composition of zooplankton and benthic invertebrates as well as ecosystem properties [@Harmon2009]. 
Their reported data on average resource abundances support my prediction that character displacement in stickleback would decrease total resource abundances [compare totals for "BL" vs. "G" in Table 1 of @Harmon2009].
For future tests, I want to emphasize that it is important to measure resource abundances over time, as I also predict that abundances will be more variable (see \textbf{Food-web stability} section).
I specifically predict that character displacement would decrease time-averaged resource abundances. 
An even more explicit test would be to apply a first-order multivariate autoregressive model to obtain equilibrium estimates of resource abundances [see equation 15 in @Ives2003].
An advantage of using this type of statistical model is that one can also estimate the effective attack rate of consumers, which I predict to be increased by character displacement in most foraging scenarios.
In designing an experiment to test this theory, it is important to keep in mind the amount of available space for consumers.
For example, a mesocosm experiment may effectively constrain resources to be in the same habitat for the consumer, in which case I predict that the shape of the tradeoff in consumer attack rates will determine the impact of character displacement (see Fig. \ref{fig:plot_fig3}a-c). 
 
Although a well-designed experiment would be an ideal test, the same methods described above could be applied to observational data from the field. 
In fact, field data may already be available that could be used to test the effects of character displacement [@Schluter2000]. 
For example, the criterion that my theory questions is the third most frequently assessed of the six criteria [see "Environmental controls" in Figure 1 of @Schluter2000]. 
The data from studies that measured resources could be used to test my prediction that character displacement decreases resource abundances. 
Although my theory calls into question part of this criterion, I agree with its intention, which was to ensure that character displacement was not due to species evolving independently to different environments [@Schluter1992].
<!--However, as I predict, resource abundances will likely differ considerably under character displacement.
Perhaps the easiest revision to this criterion is to simply delete the word "food".-->
...
<!--This theory is most applicable to situations where consumers compete for two discrete resources. Although there is certainly diversity within each resource type in natural systems,--> 
<!--
The theory presented here provides specific testable predictions for how character displacement will shape resource abundances and food-web stability. <!--First, we predict that changes in resource abundances will correspond with changes in stability. This is consistent with what we have observed here, and is a common feature of consumer-resource systems [@Murdoch2003].
Character displacement is far from an all-or-nothing process, but may vary in its magnitude [@Schluter2000].-->
<!--My results suggest that the magnitude of character displacement will be negatively correlated with total resource abundances, at least when resources occur in distinct habitats (line trajectories in Fig. \ref{fig:plot_fig3} and Fig. S1 in Appendix S4). These predictions could be tested either experimentally, by putting populations that vary in divergence in a common resource setting, or with field data that statistically controls for other factors affecting resources.
...
Testing the effect of character displacement on food-web stability will be inherently more difficult than for resource abundances. 
Fortunately, a common empirical metric of stability, the coefficient of variation in abundance (CV), corresponds to changes in the dominant eigenvalue of consumer-resource interactions [@Gellner2016TheorEcol]. 
The coefficient of variation is an indicator of temporal variability and requires data on the mean ($\mu$) and standard deviation ($\sigma$) of population abundance over time ($\text{CV}=\sigma/\mu$). 
I predict that character displacement would increase the coefficient of variation (across most foraging scenarios) in each consumer and resource population. 
As with resource abundances, a more explicit test <!--reliable, but also more data hungry approach, -->would be to apply multivariate autoregressive models to time series of consumer and resource abundances in order to directly estimate the dominant eigenvalue of the food web [see equation 22 in @Ives2003].

<!-- It is worth noting that that there could be systematic differences in consumer abundance as well due to character displacement. In fact, character displacement will likely decrease consumer abundances (ref. Fig. ), which is common in consumer-resource systems [@Abrams2019]. However, I expect that character displacement will increase consumer-resource ratios, which may be an indicator of a less stable system [].-->
<!--
In addition, I expect that the magnitude of character displacement will be positively correlated with temporal variability in consumer and resource abundances. There are two measures of temporal variability that explicitly align with the metric of food-web stability used here ($-\lambda_{max}$)--- dominant eigenvalue of a stochastic system and the coefficient of variation in consumer and resource abundances. First, the dominant eigenvalue of a food web can be estimated directly from time series of interacting populations [@Ives2003]. The dominant eigenvalue has a similar interpretation for both deterministic and stochastic models [@Ives2003], and thus could be used to test the theory. Second, changes in the coefficient of variation match deterministic changes in the dominant eigenvalue of a food web. Therefore, we expect a larger coefficient of variation in abundances with increasing character displacement. Note though, the changes in the coefficient of variation can be due to changes in both the mean and variability of species abundances, both of which we expect character displacement to alter. This theory explicitly predicts, however, that the increased variability will be driven by increased variance. In fact, we expect mean driven changes in stability to not occur in the context of character displacement (regions where stability dips toward zero in \ref{fig:plot_fig3}). This increased variability increases the risk that species will reach extremely low densities where they may collapse.  
Apparently, the "environmental controls" criteria is the third most frequently assessed of the six criteria [see Figure 1 in @Schluter2000]. For those studies that measured resources, these data could be used to test my prediction that character displacement decreases resource abundances. I do agree with the initial spirit of this criteria, which was to ensure that character displacement was not due to species evolving independently to different environments [@Schluter1992].<!-- Perhaps the easiest revision to this criterion is to delete "food".--> 

<!--
Also, detecting these changes may be easier for examples of character displacement in aquatic systems, like with threespine stickleback, given that aquatic food webs have several properties that make them less resilient to perturbations, such as higher productivity and more efficient energy transfer to consumers [@Rip2011].



<!--
In two of the three models we studied, food webs always remained locally stable. If the food-web is locally stable, then what does a less negative eigenvalue mean biologically? I argue that this comes into play in particular for stochastic systems. In stochastic systems, populations are always vulnerable to extinction, whereas this is not the case in deterministic models. Recent theory developed for food webs has shown a nice parallel between theory developed for deterministic and stochastic models in food webs. In particular, they have linked the classic checkmark pattern in stability, which we similarly observe, corresponds with the coefficient of variation in species abundances [@Gellner2016TheorEcol]. The coefficient of variation is a common empirical metric of stability, and it is nice to see that it has this close correspondence with the dominant eigenvalue of a consumer-resource system, and potentially even more complex food webs [@Gellner2016NatComm]. Therefore, we also expect that instability due to character displacement will be driven by variability in species abundances, rather than the mean. Both contribute to extinction risk at different parts of the check-mark pattern in stability [@Gellner2016TheorEcol]. However, it is also possible to estimate dominant eigenvalue of a system directly through analyses of community time series of interacting populations [@Ives2003]. Both approaches could be used to test our theoretical predictions.

testable predictions:
- instability due to character displacement will be driven by variability in species abundances, rather than the mean. Both contribute to extinction risk at different parts of the check-mark pattern in stability [@Gellner2016TheorEcol].
- in stickleback: we would expect greater temporal variability in both consumer and resource (zooplankton and benthic invertebrates) abundances, relative to its mean, in lakes where stickleback have undergone character displacements compared to those where it has not. The stickleback system would be particularly nice test, given that lakes with and without character displacement are nearby in proximity and do not different from each other in edaphic characteristics [CITE]. 
- We also expect that the magnitude of character displacement will correspondence to the magnitude of instability. 
-->
...
A strength of this approach is that it enabled me to gain analytical insight to the effects of character displacement in a more realistic foraging scenario.
This is much less tractable in quantitative genetic [@Taper1985; @McPeek2017] or explicit genetic [@Doebeli1996] models of character displacement, which is why the foraging scenarios previously examined have been limited [but see @McPeek2017]. 
A weakness, however, is that I assume a separation of time scales between ecological and evolutionary dynamics, an assumption that is becoming less tenable [@Hairston2005; @Hendry2016]. Still, models that explicitly include resource dynamics inevitably show that resource competition results in character displacement, regardless of whether a quantitative genetic or Adaptive Dynamics approach is used [@Lawlor1976; @Taper1985]. 
-->

<!-- addressing Dan's critique -->
One assumption that warrants further consideration is that I do not allow for intraspecific variation.
Models of character displacement that allow trait variation to evolve actually predict that consumers will exhibit more trait variation in the absence of a competitor [@Taper1985].
On the one hand, increased trait variation would impose a greater fitness load for the single consumer, which would weaken its top-down effect on resources [@Schreiber2011]. 
This would act to magnify my predicted effects of character displacement on resource abundances and food-web stability. 
On the other hand, if the increased trait variance has a large heritable component, then rapid evolution may generate oscillatory or chaotic dynamics in the single consumer food web [@Schreiber2011], which could negate my predicted effects of character displacement on food-web stability.
Finally, it is important to note that my conclusions only apply to food webs with biotic resources that are nutritionally substitutable. 
It would be interesting to extend these current analyses to non-substitutable resources where convergent character displacement is expected [@Abrams1987; @Fox2008].
...
This work was inspired by discussions with Seth Rudman, Dolph Schluter, Benjamin Gilbert, and Kevin McCann.
Sarah Otto provided much help in early analyses of the mathematical model. 
I would not have been able to take the theory as far as I did without her guidance and encouragement.
Matthew Osmond, Jean Philippe Gibert, and Seth Rudman provided critical feedback on an earlier draft.

...
Here, I study a model of two consumers competing for two shared resources to examine how character displacement in consumers affects resource densities and the resilience of food webs to perturbations.
...
In the simplest scenario, consumers can forage for both resources simultaneously and their feeding rates increase linearly with resource density, such that $F_{ji}(R_i)=a_{ji}R_i$ where $a_{ji}$ is the attack rate of consumer $j$ on resource $i$ [@MacArthur1972]. 
...
An equilibrium is reached when the four rates of change in the above equations are 0, and solving the system at this point gives equilibrium densities for each resource ($\hat R_1,\hat R_2$) and consumer ($\hat C_1,\hat C_2$). 
At the coexistence equilibrium (i.e., where $\hat R_1,\hat R_2, \hat C_1,\hat C_2 > 0$), I calculated resource densities and performed a linear stability analysis [@Otto2007]. 
...
Specifically, I found that, whenever character displacement increased the effective attack rate of consumers, it resulted in lower resource densities and less resilient food webs. 
...
Under this scenario, it is theoretically possible that character displacement results in increased resource densities [green lines in \ref{fig:MacArthur_Plot}].
...
In contrast, the evolutionary tradeoff that favors character displacement the most decreased resource densities. 
...
A general result emerging from our analysis was that, when consumers were competing for resources in distinct habitats, character displacement always supressed resource densities. 

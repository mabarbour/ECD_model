...
*Number of words*: `r wordcountaddin::word_count("manuscript_ECD_model.Rmd")`
...
This is because resource abundances at equilibrium are governed by a similar dynamic (derivation in Appendix S2):
<!--
...
And since evolution favors consumer divergence onto their preferred resources, the effective attack rate of consumers ($wa_{ii}+(1-w)a_{ij}$) will always increase (Fig. \ref{fig:McCann_Plot}A), resulting in lower resource abundances (Fig. \ref{fig:McCann_Plot}B) and decreased food-web stability (Fig. \ref{fig:McCann_Plot}C).

In the previous models, character displacement influences food-web stability, but all of the food webs ultimately return to a stable equlibrium (because $\lambda_{max}<0$, derivation in Appendix S1 and S2). 
In this more realistic model, however, whether the food web is locally stable depends on the parameters of both consumers and resources. 
Using Routh-Hurwitz criteria, I found that the four-species food web will transition from having a locally stable equilibrium to a limit cycle under the following conditions (derivation in Appendix S2):

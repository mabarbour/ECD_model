...
This competition perspective undermines the fact that consumer-resource interactions ultimately mediate the process of character displacement. 
...
The general result has been that, if resources are nutritionally substitutable [@Abrams1987; @Fox2008] and there is no other strong source of abundance dependence acting on consumers [@Abrams1986], then resource competition drives the adaptive divergence of competitors [@Lawlor1976; @Taper1985; \ref{fig:fig_1}]. 
...
```{r fig_1, echo=FALSE, fig.pos="H", fig.cap="\\label{fig:fig_1}**Divergent character displacement.** A textbook example of divergent character displacement is threespine stickleback in small coastal lakes of British Columbia. Sticklback consume two main resources, zooplankton and benthic invertebrates. In isolation, stickleback evolve a generalist phenotype that can take advantage of both resources. In the presence of a competitor, however, stickleback evolve specialized phenotypes that enable them to capture different resources more efficiently. A large body of theoretical work has shown that resource competition generally drives the adaptive divergence of consumers onto different resources. Currently though, we lack theoretical predictions for how this adaptive divergence affects consumer-resource interactions and food-web dynamics."}
knitr::include_graphics("ECD_model_fig_1.pdf")
```
...
where $a_{ji}$ is the attack rate of consumer $j$ on resource $i$ [@MacArthur1972]. 
This first scenario is the starting point for many models of resource competition; however, it does not reflect many food webs where consumers are mobile and their foraging behavior links resources that occur in different habitats [@McCann2005]. 
A simple extension of the above scenario takes the form:

$$F_{ji}(R_i)=w_{ji}a_{ji}R_i$$

where $w_{ji}$ represents the proportion of time consumer $j$ spends foraging in a habitat where only resource $j$ is found (i.e. habitat preference). 
Note that since $w_{ji}$ is a proportion that $w_{jj}=1-w_{ji}$. 
Finally, it is well known that consumer feeding rates often saturate at high resource abundances [@Holling1959; @Rosenzweig1963; @Murdoch2003; @McCann2011] and they do not usually spend a fixed proportion of time in a particular habitat [@McCann2005]. 
...
where consumer $i$'s feeding rate on resource $i$ is influenced by the abundance of each resource; saturates as resource abundances increase (due to handling time $h$); and consumer habitat preferences are modified by the relative abundance of resources, such that: $W_{ii}=\frac{wR_i}{wR_i+(1-w)R_j}$. 

Previous studies have analyzed the evolution of consumer attack rates in the first two foraging scenarios using an Adaptive Dynamics approach [@Otto2007], with the general result being divergent character displacement [@Lawlor1976; @Abrams1986]. 
...
I also used an Adaptive Dynamics approach to analyze character displacement in the third foraging scenario, and I too observed divergent character displacement (detailed analysis in Appendix S1 of Supplementary Information).

```{r fig_2, echo=FALSE, fig.pos="H", fig.cap="\\label{fig:fig_2}**Foraging scenarios and evolutionary tradeoffs.** I examined the effect of ecological character displacement on food-web dynamics under different assumptions of ecological foraging scenarios and evolutionary trade-offs in consumer foraging traits. Specifically, I examined whether these effects depended on whether resources occurred in the same habitat (top left) or in different habitats (top right). I also included a more realistic functional responses. In each of these foraging scenarios, I examined the effect of different evolutionary tradeoffs in attack rates."}
knitr::include_graphics("ECD_model_fig_2.pdf")
```

## Food-web Dynamics

Given that divergent character displacement was an expected result, I focus here on its consequences for food-web dynamics across these foraging scenarios.
...
To gain insight to the eco-evolutionary feedback generated by divergent character displacement, I conducted simulations using an Adaptive Dynamics approach. 
<!--
...
-->
Specifically, at each evolutionary time step, I created a mutant consumer by randomly choosing a consumer and modifying its attack rate on one resource by either subtracting or adding a small constant (0.001 in the following simulations) with equal probability. 
...
tradeoff_plot <- bind_rows(concaveDN_df, linear_df, concaveUP_df) %>%
  ...
  geom_line(size=1.5) + 
  geom_point(data = initial_points, aes(x=a11, y=a12), size=5, show.legend=FALSE) +
...
save_plot(filename = "attack_tradeoffs.pdf", plot = tradeoff_plot, base_height = 6, base_width = 9)
```
...
In this first scenario, the abundance of resources at equilibrium are equivalent when both consumers and resources are present ($\hat R = \hat R_1 = \hat R_2$), and are determined by the following equation (derived in Appendix S2):
...
-->
In this second scenario, I again see that resource abundances are equivalent when both consumers and resources are present ($\hat R = \hat R_1 = \hat R_2$), but are now determined by the following equation (derived in Appendix S3):
...
In the third foraging scenario, I observed the same general effect of divergent character displacement as the previous scenario (resources in different habitats, but linear functional response).
...
And since evolution favors consumer divergence onto their preferred resources (see Appendix S1), the effective attack rate of consumers ($wa_{ii}+(1-w)a_{ij}$) will always increase (Fig. \ref{fig:McCann_Plot}A), resulting in lower resource abundances (Fig. \ref{fig:McCann_Plot}B) and decreased food-web stability (Fig. \ref{fig:McCann_Plot}C).
...
Specifically, I found that the four-species food web will transition from having a locally stable equilibrium to a limit cycle under the following conditions (derived using Routh-Hurwitz criteria in Appendix S1):
...
Thus, character displacement always pushes the food web toward an unstable stucture in this more realistic foraging scenario. 
...
This predictable asymmetry emerges from a process of community assembly where a single consumer invades a system, reaches an evolutionary stable strategy (ESS) with a generalist phenotype, followed by the invasion of a second, more specialized, consumer. 
...
In fact, the ecological and evolutionary scenarios that favored the greatest displacement always decreased the abundance of resources. <!-- build paragraph around this topic sentence -->
...
Similarly, the evolutionary tradeoff that favored character displacement (concave up; orange lines) resulted in lower resource densities across all foraging scenarios.
Although data on the shape of the tradeoff in consumer foraging traits is scarce, two classic examples of character displacement, Darwin's finches and threespine stickleback, both appear to exhibit a tradeoff where extreme trait values increase the foraging rate of consumers [@Schluter1985; @Arnegard2014]. 
...
My most striking result was that divergent character displacement, when examined through the lens of food-web theory, made food webs less resilient to perturbations.  
...
This relative increase in interspecific interactions, combined with the natural oscillatory tendency of consumer-resource interactions [@Murdoch2003], creates a food-web structure that is less resilient to perturbations [@Chesson2008; @Rip2011; @McCann2011].

Interestingly, the ecological conditions that favor divergent character displacement are those that are already the least resilient to perturbations.
...
In my model, this corresponds to higher values of $K$ (productivity) or $A$ (capacity to find and capturing resources), or lower values of $m$ (abiotic stress).
...
A handful of empirical patterns support the hypothesis that character displacement decreases food-web resilience. 
As an example, a single species of threespine stickleback lives in hundreds of postglacial lakes of British Columbia, but the species pair <!--, where character displacement has happened,-->are only known from six lakes in four independent watersheds [@Schluter1992; @Schluter2000].
...
My results contrast, but do not necessarily contradict, the notion from competition theory that character displacement contributes to species coexistence [@Lawlor1976]. 
Rather than studying resilience, competition theory usually studies the mutual ability of consumers with different phenotypes to invade when rare [mutual invasibility, @Chesson2000].
...
The Adaptive Dynamics approach I used here has its strengths and weaknesses.
A strength of this approach is that it enabled me to gain analytical insight to the effects of character displacement on food-web dynamics in a more realistic foraging scenario.
...
This is because models that explicitly include resource dynamics inevitably show that resource competition results in divergent character displacement, regardless of whether a quantitative genetic or Adaptive Dynamics approach is used [@Lawlor1976; @Taper1985]. 
A quantitative genetic model may certainly show differences in the magnitude of character displacement, but this should not affect the general relationships I observed.  
...
I want to note though, that my conclusions only apply to food webs with biotic resources that are nutritionally substitutable. 

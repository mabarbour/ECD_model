...
params_2C_2R_MacArthur_concaveDN_asymm[c("a21","a22")] <- sim_1C_2R_MacArthur_concaveDN[nrow(sim_1C_2R_MacArthur_concaveDN), c("a12","a11")] # replace C2's attack rates with the ending point of the C1 generalist simulation. This will create an asymmetry in the attack rates and follows the logic (and our results) that a likely scenerio with asymmetry is a specialist invading a generalist.
# just altering the parameters, everything else stays the same
...
params_2C_2R_MacArthur_concaveUP_asymm[c("a21","a22")] <- sim_1C_2R_MacArthur_concaveUP[nrow(sim_1C_2R_MacArthur_concaveUP), c("a12","a11")] # replace C2's attack rates with the ending point of the C1 generalist simulation. This will create an asymmetry in the attack rates and follows the logic (and our results) that a likely scenerio with asymmetry is a specialist invading a generalist.
# just altering the parameters, everything else stays the same
...
# asymmetry with C1 starting with a different habitat preference (w)
#params_1C_2R_McCann_linear_switch <- params_1C_2R_McCann_linear # NEED TO FIGURE OUT WHAT TO DO. i SHOULD PROBABLY MAKE SURE ATTACK RATES AND PREFERENCE ARE ALIGNED, BUT I SHOULD JUST DO THAT FOR THE ALTERNATIVE RESOURCE. E.G. C1 SPECIALIZING ON R2 INSTEAD OF R1 IN THE C1 ONLY SIMS.
# switching direction of pre-adapted attack rates and habitat preference
#params_1C_2R_McCann_linear_switch["a11"] <- params_1C_2R_McCann_linear["a12"] 
#params_1C_2R_McCann_linear_switch["a12"] <- params_1C_2R_McCann_linear["a11"] 
#params_1C_2R_McCann_linear_switch["w11"] <- params_1C_2R_McCann_linear["w12"] 
#params_1C_2R_McCann_linear_switch["w12"] <- params_1C_2R_McCann_linear["w11"]
#sim_1C_2R_McCann_linear_switch <- eco_evo_CR(init_parameters = params_1C_2R_McCann_linear_switch, 
#                                             init_states = states_1C_2R, 
#                                             eco_CR_model = McCann_1C_2R, 
#                                             AD_CR_models = c(mC1_McCann),
#                                             species_traits = c(1), 
...
#                                             tradeoff_functions = c(a12_tradeoff),
#                                             extra_tradeoff_params = c(A=A, n=n_linear))
#params_2C_2R_McCann_linear_asymm_switch <- params_2C_2R_McCann_linear
#params_2C_2R_McCann_linear_asymm_switch[c("a21","a22")] <- sim_1C_2R_McCann_linear_switch[nrow(sim_1C_2R_McCann_linear_switch), c("a11","a12")] 
#sim_2C_2R_McCann_linear_asymm_switch <- eco_evo_CR(init_parameters = params_2C_2R_McCann_linear_asymm_switch,
#                                                    init_states = states_2C_2R, 
#                                                    eco_CR_model = McCann_2C_2R, 
#                                                    AD_CR_models = c(mC1_McCann, mC2_McCann),
#                                                    species_traits = c(1,2), 
...
sim_2C_2R_McCann_concaveDN_asymm <- eco_evo_CR(init_parameters = params_2C_2R_McCann_concaveDN_asymm,
                                ...
                                tradeoff_functions = c(a12_tradeoff, a21_tradeoff),
                                extra_tradeoff_params = c(A=A, n=n_concaveDN))
# asymmetric starting points | switched
...
LS_attack_asymm <- ggplot(tradeoff_df, aes(x=aii/(aii+aij), y=aii+aij, color = factor(n))) + 
  geom_line() +
  geom_point(data = filter(LawlorSmith_displacements_asymm, Time=="End"), aes(x = a11/(a11+a12), y=a11+a12, fill=factor(n), shape=Competitor), size=3, color="white") + #, show.legend=FALSE) + size=5,
  geom_point(data = filter(LawlorSmith_displacements_asymm, Time=="Begin"), aes(x = a11/(a11+a12), y=a11+a12, fill=factor(n), shape=Competitor), size=1.5, color="white") +
  ylab(expression(a[ii]+a[ij])) +
  xlab(expression(frac(a[ii],a[ii]+a[ij]))) +
  scale_shape_manual(values=c(24,21)) +
  scale_color_manual(name="Tradeoff (n)", values=cbPalette)  +
...
```{r LS_Plot_asymm, echo=FALSE, fig.cap="\\label{fig:LS_Plot_asymm}Effect of character displacement when consumers *cannot* forage for both resources simultaneously. Different line colors correspond to the tradeoff values shown in fig. 1A. Large circles (two consumers) and triangles (one consumer) correspond to the end points of the eco-evolutionary simulation, whereas as small shapes correspond to the starting points (only in panel C). Panel (A) shows how, regardless of the tradeoff, character displacement increases the effective attack rate of the consumer community. This always resulted in a suppression of resource densities (B) and concomitant decrease in food-web stability (C). As in fig. 1, our simulations used the following parameter values: *r* = 1; *K* = 4; *e* = 0.8; *m* = 1; *A* = 2. We set an initial value of *a~ii~* = 1.2 and *a~ij~* depended on the value of *n*. We set initial consumer and resource abundances as: *R~i~* = *R~j~* = 2; *C~i~* = *C~j~* = 1."}
...
McCann_attack_asymm <- ggplot(tradeoff_df, aes(x=aii/(aii+aij), y=aii+aij, color = factor(n))) + 
  geom_line() +
  geom_point(data = filter(McCann_displacements_asymm, Time=="End"), aes(x = a11/(a11+a12), y=a11+a12, fill=factor(n), shape=Competitor), size=3, color="white") + #, show.legend=FALSE) + size=5,
  geom_point(data = filter(McCann_displacements_asymm, Time=="Begin"), aes(x = a11/(a11+a12), y=a11+a12, fill=factor(n), shape=Competitor), size=1.5, color="white") +
  ylab(expression(a[ii]+a[ij])) +
  xlab(expression(frac(a[ii],a[ii]+a[ij]))) +
  scale_shape_manual(values=c(24,21)) +
  scale_color_manual(name="Tradeoff (n)", values=cbPalette)  +
...
```{r McCann_Plot_asymm, echo=FALSE, fig.cap="\\label{fig:McCann_Plot_asymm}Effect of character displacement when consumers exhibit a more realistic functional response. Different line colors correspond to the tradeoff values shown in fig. 1A. Large circles (two consumers) and triangles (one consumer) correspond to the end points of the eco-evolutionary simulation, whereas as small shapes correspond to the starting points (only in panel C). Panel (A) shows how, regardless of the tradeoff, character displacement increases the effective attack rate of the consumer community. This always resulted in a suppression of resource densities (B) and concomitant decrease in food-web stability (C). As in fig. 1, our simulations used the following parameter values: *r* = 1; *K* = 4; *e* = 0.8; *m* = 1; *A* = 2. We set an initial value of *a~ii~* = 1.2 and *a~ij~* depended on the value of *n*. We set initial consumer and resource abundances as: *R~i~* = *R~j~* = 2; *C~i~* = *C~j~* = 1."}

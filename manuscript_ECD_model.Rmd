...
Controlling for other sources of variability allowed me to isolate the general effects of divergent character displacement.
All mathematical derivations were conducted in @Mathematica.
...
This function has the useful property that it differentiates between cases where intermediate combinations of $a_{ii}$ and $a_{ij}$ are higher than the extremes (when $n>1$, green line in Fig. \ref{fig:fig_1}) or, conversely, where the two extremes are higher than intermediate investments (when $n<1$, orange line Fig. \ref{fig:fig_1}). 
When $n=1$, the tradeoff function is linear, and all combinations of $a_{ii}$ and $a_{ij}$ have the same total attack rate (blue line in Fig. \ref{fig:fig_1}). 
...
Thus, divergent character displacement always results in resource suppression (Fig. \ref{fig:plot_fig3}). 
...
Similarly, the evolutionary tradeoff that favored character divergence resulted in lower resource densities across all foraging scenarios.

...
# plot the dynamics
matplot.0D(ode(species_pair.init.states, 1:500, ECD_model, species_pair.parms), ylim=c(0,2))#K_scale))
...
# plot the dynamics
matplot.0D(ode(solitary.init.states, 1:500, ECD_model.C1.3sp, solitary.parms), ylim=c(0,2)) #K_scale))
...
# as expected, total resource abundances are lower in species pair lakes (but not by much)
(species_pair_dynamics$y["R1"] + species_pair_dynamics$y["R2"])/(solitary_dynamics$y["R1"] + solitary_dynamics$y["R2"]) - 1 

# as expected, zooplankton densities are lower in species-pair lakes
species_pair_dynamics$y["R1"]/solitary_dynamics$y["R1"] - 1 # zooplankton densities are lower in species pair lakes

# but why are benthic invert densities a lot higher in species pair lakes?
species_pair_dynamics$y["R2"]/solitary_dynamics$y["R2"] - 1 

## Consumers

# total consumer densities are lower in species pair lakes
(species_pair_dynamics$y["C1"] + species_pair_dynamics$y["C2"])/solitary_dynamics$y["C1"] - 1

# limnetics are suspiciously more abundant than benthics
species_pair_dynamics$y["C1"]/species_pair_dynamics$y["C2"]
  
## Consumer:Resource Ratios

# C:R ratios are larger in solitary vs. species-pair lakes, which is a bit unexpected. They do fall within the range of C:R biomass ratios in aquatic ecosystems though Fig. 3 Rip & McCann 2011
